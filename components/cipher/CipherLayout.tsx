...
import { clampStepIndex, updateStepInCurrentUrl } from "@/lib/utils/visualizerPermalink";
...
export default function CipherLayout({ cipher }: CipherLayoutProps) {
  ...
  const options: CipherOptions = useMemo(
    () => ({
      hexInput,
      rounds,
      demoMode,
      bobSecret,
      aesMode,
      padding: padding ? "pkcs7" : "none",
      autoCompute,
    }),
    [hexInput, rounds, demoMode, bobSecret, aesMode, padding, autoCompute]
  );

  const permalinkOptions = useMemo(
    () =>
      ({
        hexInput,
        rounds,
  ...
  const restorePermalink = useCallback(
    (shared: ReturnType<typeof import("@/lib/utils/visualizerPermalink").parseVisualizerPermalink>) => {
      ...
      if (shared.step !== undefined) {
        setCurrentStep(shared.step);
        pendingSharedStepRef.current = shared.step;
      }
    },
    [cipher.id]
  );

  const { setStep } = usePermalinkController({
    cipherId: cipher.id,
    input,
    key,
    action,
  ...
  const handlePresetLoad = (
    preset: import("@/lib/utils/workspacePresets").WorkspacePreset
  ) => {
...
const virtualizedResult = result;  const activeStep = result?.steps?.[currentStep];
  ...
  const provenance = resolveProvenance(
    result?.metadata?.provenance ??
      (cipher.id === "dh" || (cipher.id === "rsa" && demoMode)
        ? ({ provenance: "simulated", source: "CryptoViz educational simulation" } as DataProvenanceMetadata)
        : undefined)
  );
      ...
      <div className="grid grid-cols-1 items-start gap-5 md:gap-6 lg:grid-cols-12 lg:gap-8">
        <div className="flex flex-col gap-6 lg:col-span-5">
          ...
          <CipherOptionsPanel
            ...
            onAutoComputeChange={setAutoCompute}
            onHexInputChange={setHexInput}
            onRoundsChange={setRounds}
            onDemoModeChange={setDemoMode}
            onBobSecretChange={setBobSecret}
          ...
          {(error || workerError) && (
            <div
              role="alert"
              aria-live="polite"
              className="rounded-xl border border-red-100 bg-red-50 p-4 text-xs text-red-700 dark:border-red-950/40 dark:bg-red-950/10 dark:text-red-400"
            >
        ...
        <div className="flex flex-col gap-6 lg:col-span-7">
          <div
            className="flex rounded-lg bg-zinc-100 p-0.5 dark:bg-zinc-800/80"
            role="tablist"
            aria-label="Cipher result views"
          >
            {(() => {
              const tabs: Array<"result" | "history" | "debugger"> = ["result", "history"];

              if (["aes", "des", "3des", "twofish", "serpent", "camellia", "aria"].includes(cipher.id)) {
                tabs.push("debugger");
              }
          ...
          {activeTab === "result" && (
            <>
              ...
              >
                ...
                <div className="mt-2 min-h-[48px] overflow-x-auto rounded-lg bg-zinc-50 p-3 font-mono text-sm break-all dark:bg-zinc-950/40">
                  {loading ? (
                    "Computing..."
                  ) : result ? (
                    result.output
              ...
              {result?.steps?.length ? (
                <section aria-label="Step-by-step trace" className="flex flex-col gap-4">
                  <div className="flex items-center justify-between px-1">
                    ...
                    <button
                      ...
                      onClick={() =>
                        ...
                        activeStepId &&
                        setAnnotationStore(
                          toggleStepBookmark(annotationStore, annotationScope, activeStepId, activeStep.label)
                        )
                      }
...
stepMetadata={
  "stepMetadata" in result
    ...
    : undefined
                    ...
                    onCopyStepLink={async () => setStep(currentStep)}
                  />
                </section>
              ) : null}
            </>
