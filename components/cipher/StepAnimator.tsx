...
export default function StepAnimator({ steps, currentStep, onStepChange }: StepAnimatorProps) {
  ...
  return (
    <div ref={stepRef} className="rounded-xl border border-zinc-200 bg-white p-4 shadow-sm dark:border-zinc-800 dark:bg-zinc-900/50">
      <div className="flex items-center justify-between border-b border-zinc-100 pb-3 dark:border-zinc-800">
        <div className="flex items-center gap-2 min-w-0">
          <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-teal-50 text-xs font-bold text-teal-700 dark:bg-teal-950/50 dark:text-teal-400">
            {currentStep + 1}
          </span>
          <h4 className="font-semibold text-zinc-900 dark:text-white truncate">{step.label}</h4>
        </div>

        {step.isMilestone && (
          <span className="shrink-0 rounded-full bg-teal-50 px-2 py-0.5 text-2xs font-semibold uppercase tracking-wider text-teal-700 dark:bg-teal-950/50 dark:text-teal-400">
            Milestone
          </span>
        )}
      </div>
      ...
      <div className="py-4">
        ...
        {step.table && step.table.length > 0 && (
          <div className="mt-3 overflow-x-auto rounded-lg border border-zinc-150 dark:border-zinc-800">
            <table className="w-full text-left text-xs font-mono">
              <thead className="bg-zinc-50 text-zinc-500 dark:bg-zinc-950/40 dark:text-zinc-500">
                <tr>
                  <th className="px-3 py-1.5 font-semibold whitespace-nowrap">Parameter</th>
              ...
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {step.table.map((row, idx) => (
                  <tr key={idx} className="bg-white dark:bg-zinc-900/10">
                    <td className="px-3 py-1.5 font-medium text-zinc-500 dark:text-zinc-400 whitespace-nowrap">{row.key}</td>
                    <td className="px-3 py-1.5 break-all text-zinc-900 dark:text-zinc-200 font-mono">{row.value}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      <div className="mt-2 flex flex-col gap-3 border-t border-zinc-100 pt-3 dark:border-zinc-800 sm:flex-row sm:items-center">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            aria-label={isPlaying ? 'Pause' : 'Play Auto-Advance'}
          >
            {isPlaying ? (
