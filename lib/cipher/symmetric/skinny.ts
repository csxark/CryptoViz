...
// Authentic SKINNY 8-bit S-box S8 (EUROCRYPT 2016 specification)
const SBOX: number[] = [
...
function skinnyCore(input: string, key: string, doDecrypt: boolean, options: CipherOptions): CipherResult {
    ...
    if (keyBytes.length !== tkSize) throw new CipherError('INVALID_KEY_LENGTH', `INVALID_KEY_LENGTH: Tweakey must be ${tkSize} bytes for ${paramSet}.`)
    ...
    // Precompute round tweakeys and round constants
    const roundTKs: number[][] = []
    const roundRCs: number[] = []
    let rc = 0x01
    for (let r = 0; r < rounds; r++) {
        const rtk: number[] = []
        for (let i = 0; i < 8; i++) {
            rtk.push(u8(TK1[i] ^ TK2[i] ^ (tkSize >= 48 ? TK3[i] : 0)))
        }
        roundTKs.push(rtk)
        roundRCs.push(rc)
    ...
    if (!doDecrypt) {
        for (let r = 0; r < rounds; r++) {
            ...
            addConstants(state, roundRCs[r])
            for (let i = 0; i < 8; i++) {
                state[i] = u8(state[i] ^ roundTKs[r][i])
            }
            shiftRows(state, false)
            mixColumns(state, false)

            if (r % 10 === 0) {
                steps.push({ index: steps.length, label: `Round ${r + 1} — SKINNY SPN`, inputState: toHex(inBytes), outputState: toHex(state), note: `TWEAKEY TK1/TK2/TK3 updated. RC=${roundRCs[r].toString(16)}` })
            }
        }
    } else {
        for (let r = rounds - 1; r >= 0; r--) {
            ...
            if (r % 10 === 0) {
                steps.push({ index: steps.length, label: `Round ${r + 1} (inv) — SKINNY SPN`, inputState: toHex(inBytes), outputState: toHex(state), note: `Inverse SKINNY SPN round. RC=${roundRCs[r].toString(16)}` })
            }
        }
    }
